...
- 2026-06-10: Implemented the file association: `CFBundleDocumentTypes` (md/markdown/mdown) + stable `appBundleId` in the packaged Info.plist, and an `app.on("open-file")` handler covering before-ready (stash and render at startup), while-running (re-render in place, title update), and windows-closed (recreate window) deliveries. Verified both event paths against the packaged app using LaunchServices `open` (the same `odoc` event Finder double-click sends): warm swap-in-place and cold launch-with-document both render. Remaining HITL is just Finder/Gatekeeper: copy to /Applications, right-click → Open once, double-click a `.md`. Checklist in `NOTES-HITL.md`.
- 2026-06-10: HITL verification passed (human): packaged app works from /Applications and document opens are fast. One residual wrinkle: Finder's "Always open with → MarkRover" does not persist as the `.md` default (Xcode keeps reclaiming it) — almost certainly because the bundle is unsigned; LaunchServices on current macOS will not persist a default-handler claim for an unsigned app, and the duplicate registration from the `out/` build copy may compound it. Candidate fixes: ad-hoc `codesign --force --deep -s -` on the /Applications copy plus `lsregister` cleanup, or real Developer ID signing (already a known follow-up).
- 2026-06-10: Closing as completed. `package:mac` now ad-hoc signs the bundle after packaging so LaunchServices will persist the `.md` default-handler claim. NOTES-HITL.md deleted; its surviving follow-ups, now tracked here: (1) the ditto unzip patch is darwin-only — make it conditional before Windows/Linux packaging; (2) bundle is ~261 MB, dominated by lazy Mermaid/katex/cytoscape renderer chunks — audit before distribution; (3) ad-hoc signing is local-only — Developer ID signing + notarization required for distribution.

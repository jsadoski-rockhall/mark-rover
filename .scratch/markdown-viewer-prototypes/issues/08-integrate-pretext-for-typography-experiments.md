...
- 2026-06-09: Branch `issue-08-pretext-typography`. Added pretext-assisted narrow-window leading: when the window truncates the selected measure, line-height scales down toward a 1.45 floor in proportion to how much of the measure actually fits (glyph-accurate via `measureNaturalWidth`). Behind the same probe off-switch; basic rendering unaffected. Proposed ADR-0001 (`docs/adr/0001-pretext-as-an-optional-typography-layer.md`) pinning pretext as an optional enhancement layer and documenting Canvas 2D / `Intl.Segmenter` / system-font / shaping limitations.
